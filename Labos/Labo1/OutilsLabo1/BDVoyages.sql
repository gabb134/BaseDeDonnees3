...
CREATE TABLE faitLeVoyage(
  ...
  CONSTRAINT fk_faitVoyageClient FOREIGN KEY(cliNo) REFERENCES client(cliNo)
	...
	select 'Nombre d''hôtels à Montréal: '+convert(varchar,count(hotNom)) from hotel 
	where hotNom like '%Montréal%'
	...
	--e
	--	select voyNo, voyDateDepart,voyDateArrive,voyDestination from voyage where voyDestination = 'Montréal'
	...
	begin transaction
	go
	select * from voyage where voyDestination = 'Montréal'

	--update voyage set tarifSimple = tarifTriple * 1.10 where voyDestination = 'Montréal'
		
	select tarifSimple from voyage where voyDestination = 'Montréal'
	...
	select empNo,count(voyActivite) from voyage group by empNo
